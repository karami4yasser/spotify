...
import { VolumeUpIcon, HeartIcon } from "@heroicons/react/outline";
import {
  RewindIcon,
  PauseIcon,
  PlayIcon,
...
function Player() {
    ...
    spotifyApi.getMyCurrentPlaybackState().then((data) => {
      if (data.body?.is_playing) {
        spotifyApi.pause().catch((err) => {
          console.log("YOU NEED A PREMIUM ACCOUNT TO PLAY");
          setIsPlaying(false);
        });
        setIsPlaying(false);
      } else {
  ...
  const debouncedAdjustVolume = useCallback(
    //we dont call te api au each modification but we wait
    debounce((volume) => {
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-4">
        ...
        <button>
          <HeartIcon className="w-5 h-5 cursor-pointer hover:scale-125 transition transform duration-100 ease-out " />
        </button>
      </div>
